...
import com.zymr.zvisitor.util.DefensePointUtil;
...
public class VisitorService {
	...
	private void saveVisitorImages(MultipartFile visitorImage, MultipartFile visitorSignature, Path saveDir) throws IOException {
		...
		if (Objects.nonNull(visitorImage)) {
			String profilePicPath = Util.getImagePath(FilenameUtils.getName(visitorImage.getOriginalFilename()), imageService.getBaseDirPathAsStr(), saveDir.toString()).toString();
		...
		if (StringUtils.isNotBlank(signaturePicPath)) {
			if(!DefensePointUtil.isSubDirectory(signaturePicPath))
				try {
